...
class _Packing(object): 
    ...
    def add_job(self, rpacking):
        self.packing_jobs.extend(rpacking.packing_jobs)

    def calc_iteration_time(self, packing=None, ordering=1, calc_weight=False):
        TT_all = float("inf")
        if packing!=None:
        ...
        if calc_weight:
            return TT_all
        if job_cnt == 2:
            TT_all *= JOBS.itertime_overhead_list[1]
        elif job_cnt == 3:
            TT_all *= JOBS.itertime_overhead_list[2]
        elif job_cnt == 4:
            TT_all *= JOBS.itertime_overhead_list[3]
        return TT_all
    ...
    def calc_weight(self, packing):
        jobs = self.packing_jobs + packing.packing_jobs
        TT = self.calc_iteration_time(packing, calc_weight=True)
        min_val = float('inf')
        minx = 0
        for rjob in jobs:
...
class _Blossom_Same(object):
    ...
    def set_match(self, u, v):
        ...
        while not set_queue.empty():
            ...
            pr = self.get_pr(u, xr)
            for i in range(pr):
                set_queue.put((self.flower[u][i], self.flower[u][i^1]))
            set_queue.put((xr, v))
            tmp_flower = self.flower[u][pr:]+self.flower[u][:pr]
            self.flower[u] = tmp_flower

    # set the match of u to be v; specially deal with u when u is a flower
    def set_match_backup(self, u, v):
        self.match[u] = self.edge[u][v]["v"]
        if u<= self.node_cnt:
            return
        e = self.edge[u][v]
    ...
    def augment(self, u, v):
        ...
        while not queue_tmp.empty():
            ...
            queue_tmp.put((self.st[self.pa[xnv]], xnv))
    
    def augment_backup(self, u, v):
        xnv = self.st[self.match[u]]
        self.set_match(u,v)
        if xnv==0:
            return
